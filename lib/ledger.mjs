...
/** Credits a paid Alipay order exactly once and lets the caller update its
 * payment row in the same SQLite transaction. */
export function creditAlipayPurchase(userId, outTradeNo, amountMicro, { onCredited = null } = {}) {
  return withUserLock(userId, () => tx(() => {
    if (!outTradeNo) throw Object.assign(new Error('支付宝商户订单号不能为空'), { statusCode: 400 });
    if (!Number.isSafeInteger(amountMicro) || amountMicro <= 0) throw Object.assign(new Error('到账积分无效'), { statusCode: 400 });
    const idempotencyKey = `alipay-payment-${outTradeNo}`;
    const existing = findEntry(userId, idempotencyKey);
    const user = selectUser(userId);
    if (!user) throw Object.assign(new Error('账号不存在'), { statusCode: 404 });
    if (existing) {
      onCredited?.({ replay: true, entry: existing, balanceMicro: user.balanceMicro });
      return { ...snapshot(user.balanceMicro, user.heldMicro), entry: existing, replay: true };
    }

    const balanceMicro = user.balanceMicro + amountMicro;
    if (!Number.isSafeInteger(balanceMicro)) throw Object.assign(new Error('到账后积分余额超出安全范围'), { statusCode: 409 });
    writeWallet(userId, balanceMicro, user.heldMicro);
    const entry = appendEntry(userId, idempotencyKey, {
    ...
    onCredited?.({ replay: false, entry, balanceMicro });
    return { ...snapshot(balanceMicro, user.heldMicro), entry, replay: false };
  }));
}

export function reserveAlipayRefundCredits(userId, outRequestNo, amountMicro) {
  return withUserLock(userId, () => tx(() => {
    if (!outRequestNo || !Number.isSafeInteger(amountMicro) || amountMicro <= 0) throw Object.assign(new Error('退款积分冻结参数无效'), { statusCode: 400 });
    const existing = selectHold(userId, outRequestNo);
    if (existing) return existing;
    const user = selectUser(userId);
    if (!user) throw Object.assign(new Error('账号不存在'), { statusCode: 404 });
    const availableMicro = Math.max(0, user.balanceMicro - user.heldMicro);
    if (availableMicro < amountMicro) throw Object.assign(new Error('可用积分不足，无法发起退款'), { statusCode: 409 });
    const createdAt = now();
    const hold = { id: outRequestNo, type: 'alipay_refund', status: 'held', reservedMicro: amountMicro, chargedMicro: null, createdAt, updatedAt: createdAt };
    writeWallet(userId, user.balanceMicro, user.heldMicro + amountMicro);
    writeHold(userId, hold);
    return hold;
  }));
}

export function captureAlipayRefundCredits(userId, outRequestNo) {
  return withUserLock(userId, () => tx(() => {
    const hold = selectHold(userId, outRequestNo);
    if (!hold) throw Object.assign(new Error('退款积分冻结记录不存在'), { statusCode: 409 });
    const user = selectUser(userId);
    if (!user) throw Object.assign(new Error('账号不存在'), { statusCode: 404 });
    const idempotencyKey = `alipay-refund-${outRequestNo}`;
    const existing = findEntry(userId, idempotencyKey);
    if (existing || hold.status === 'captured') return { ...snapshot(user.balanceMicro, user.heldMicro), replay: true };
    if (hold.status !== 'held' || user.heldMicro < hold.reservedMicro || user.balanceMicro < hold.reservedMicro) {
      throw Object.assign(new Error('退款积分冻结状态异常'), { statusCode: 409 });
    }
    const balanceMicro = user.balanceMicro - hold.reservedMicro;
    const heldMicro = user.heldMicro - hold.reservedMicro;
    writeWallet(userId, balanceMicro, heldMicro);
    appendEntry(userId, idempotencyKey, {
      type: 'alipay_refund',
      amountMicro: -hold.reservedMicro,
      balanceAfter: microToCredits(balanceMicro),
      balanceAfterMicro: balanceMicro,
      externalRef: outRequestNo,
      note: '支付宝退款扣回积分',
    });
    hold.status = 'captured';
    hold.chargedMicro = hold.reservedMicro;
    hold.updatedAt = now();
    writeHold(userId, hold);
    return { ...snapshot(balanceMicro, heldMicro), replay: false };
  }));
}

export function releaseAlipayRefundCredits(userId, outRequestNo) {
  return withUserLock(userId, () => tx(() => {
    const hold = selectHold(userId, outRequestNo);
    const user = selectUser(userId);
    if (!hold || !user || hold.status !== 'held') return user ? snapshot(user.balanceMicro, user.heldMicro) : null;
    writeWallet(userId, user.balanceMicro, Math.max(0, user.heldMicro - hold.reservedMicro));
    hold.status = 'released';
    hold.updatedAt = now();
    writeHold(userId, hold);
    return snapshot(user.balanceMicro, Math.max(0, user.heldMicro - hold.reservedMicro));
  }));
}
...
export function reserveLlmCredits(userId, requestId, reservedMicro, metadata = {}) {
  return withUserLock(userId, () => tx(() => {
    ...
    const hold = {
      ...
      ...metadata, llmRates:{ ...config.llmRates }, createdAt: now(), updatedAt: now(),
    };
...
export function settleLlmCredits(userId, requestId, result, metadata = {}) {
  return withUserLock(userId, () => tx(() => {
    ...
    const rates = hold.llmRates || config.llmRates;
    const actualMicro = llmCostMicro(result.usage.inputTokens, result.usage.outputTokens, rates);
    ...
    const usage = {
      ...
      inputRateYuanPerMillion: rates.inputYuanPerMillion,
      outputRateYuanPerMillion: rates.outputYuanPerMillion,
...
export function chargeGenerationBatchMicro(userId, items) {
  return withUserLock(userId, () => tx(() => {
    if (!Array.isArray(items) || !items.length) throw new Error('批量生成任务不能为空');
    const user = selectUser(userId);
    if (!user) return { status: 401, error: '账号不存在' };
    const pending = [];
    let totalMicro = 0;
    ...
    for (const item of items) {
      ...
      const costMicro = item?.costMicro;
      if (!generationId || ids.has(generationId)) throw new Error('批量生成任务 ID 无效');
      ids.add(generationId);
      if (!Number.isSafeInteger(costMicro) || costMicro < 0) throw new Error('生成费用无效');
      if (findEntry(userId, `charge-${generationId}`)) continue;
      totalMicro += costMicro;
      if (!Number.isSafeInteger(totalMicro)) throw new Error('批量生成费用超出安全范围');
      pending.push({ ...item, generationId, costMicro });
    }

    const availableMicro = Math.max(0, user.balanceMicro - user.heldMicro);
    if (availableMicro < totalMicro) {
      return {
        status: 402,
        error: `积分不足，本次需要 ${microToCredits(totalMicro)} 积分，当前可用 ${microToCredits(availableMicro)} 积分`,
        balance: microToCredits(user.balanceMicro),
        available: microToCredits(availableMicro),
      };
    }

    let balanceMicro = user.balanceMicro;
    for (const item of pending) {
      const { onCharged, ...entryMetadata } = item.metadata || {};
      balanceMicro -= item.costMicro;
      appendEntry(userId, `charge-${item.generationId}`, {
        type: 'generation_charge', amountMicro: -item.costMicro,
        balanceAfter: microToCredits(balanceMicro), balanceAfterMicro: balanceMicro,
        generationId: item.generationId, ...entryMetadata, note: entryMetadata.note || '生成任务预扣费',
      });
      onCharged?.();
    }
    if (totalMicro) writeWallet(userId, balanceMicro, user.heldMicro);
    return snapshot(balanceMicro, user.heldMicro);
  }));
}
