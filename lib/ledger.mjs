...
function appendEntry(userId, idempotencyKey, entry) {
  ...
  sql(`
    ...
    VALUES(:id, :userId, :idempotencyKey, :type, :amountMicro,
           :balanceAfterMicro, :generationId, :requestId, :createdAt, :docJson)`).run({
    id: record.id,
    userId,
    idempotencyKey,
    type: record.type,
...
export function chargeGeneration(userId, generationId, cost) {
  return withUserLock(userId, () => tx(() => {
    ...
    const costMicro = creditsToMicro(cost);
    const availableMicro = Math.max(0, user.balanceMicro - user.heldMicro);
    if (availableMicro < costMicro) {
      return {
        status: 402,
        error: `积分不足，本次需要 ${cost} 积分，当前可用 ${microToCredits(availableMicro)} 积分`,
...
export function refundGeneration(userId, generationId, cost) {
  return withUserLock(userId, () => tx(() => {
    ...
    // Replaying a refund must never pay out twice.
...
export function recentCreditEntries(userId, limit = 20) {
  return sql(`
    ...
    LIMIT :limit`).all({ userId, limit }).map(row => JSON.parse(row.doc_json));
}
