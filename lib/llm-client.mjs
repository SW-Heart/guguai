...
export async function callLlm({ system, prompt, content = null, maxOutputTokens = 4096, jsonMode = false, outputSchema = null, toolName = 'submit_result', thinking = outputSchema ? 'disabled' : '', config = llmConfigFromEnv(), fetchImpl = fetch }) {
  ...
  const userContent = Array.isArray(content) && content.length
    ? [{ type:'text', text:String(prompt || '') }, ...content].map(part => {
      if (!anthropic || part?.type !== 'image_url') return part;
  ...
  const body = anthropic
    ? { model: config.model, max_tokens: maxOutputTokens, system, messages: [{ role: 'user', content: userContent }] }
    : { model: config.model, max_tokens: maxOutputTokens, messages: [{ role: 'system', content: system }, { role: 'user', content: userContent }], ...(thinking ? { thinking:{ type:thinking } } : {}), ...(structuredTool ? { tools:[structuredTool], tool_choice:{ type:'function', function:{ name:toolName } } } : jsonMode ? { response_format:{ type:'json_object' } } : {}) };
  ...
  if (!response.ok) throw Object.assign(new Error(providerError(value, `LLM 请求失败（HTTP ${response.status}）`)), { statusCode: 502, upstreamError: true });
  ...
  if (!normalized.text) {
    ...
    throw Object.assign(new Error(`LLM 没有返回可用结果${suffix}`), { statusCode: 502, upstreamError: true, providerResponse:value });
  }
  if (!Number.isSafeInteger(normalized.usage.inputTokens) || normalized.usage.inputTokens < 0 || !Number.isSafeInteger(normalized.usage.outputTokens) || normalized.usage.outputTokens < 0) {
    throw Object.assign(new Error('LLM 成功响应缺少有效 Token usage，需人工核账'), { statusCode: 502, upstreamError: true, billingReconcileRequired: true, providerResponse: value });
  }
  return normalized;
}
