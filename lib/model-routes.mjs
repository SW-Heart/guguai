...
export function publicModelPrices() {
  return Object.keys(platformCapabilities).flatMap(logicalModelId => ['480p', '720p'].map(quality => {
    ...
    return { modelId: logicalModelId, label, quality, duration: platformCapabilities[logicalModelId].duration, available: Boolean(route), credits: route?.salePriceCredits ?? null, yuan: route?.salePriceYuan ?? null, selectedRouteId: route?.id || '', selectedRouteName: route?.displayName || '', priceVersion: publicRoutePriceVersion(route) };
  }));
}
