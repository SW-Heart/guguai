...
// 1 分成本 × 1.2 ÷ 0.1 元/积分 = 0.12 积分 = 120,000 microcredits。
...
export function publicModelPrices() {
  return Object.keys(platformCapabilities).flatMap(logicalModelId => ['480p', '720p'].map(quality => {
    ...
    return { modelId: logicalModelId, label: logicalModelId === 'seedance-2.0' ? 'Seedance 2.0' : 'Seedance 2.5', quality, duration: platformCapabilities[logicalModelId].duration, available: Boolean(route), credits: route?.salePriceCredits ?? null, yuan: route?.salePriceYuan ?? null, selectedRouteId: route?.id || '', selectedRouteName: route?.displayName || '', priceVersion: route ? `${route.id}:${route.version}` : '' };
  }));
}
