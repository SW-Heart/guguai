...
function channelById(id) { return MODEL_ROUTE_CHANNELS.find(item => item.id === id) || null; }
...
export function listModelRouteChannels() {
  return MODEL_ROUTE_CHANNELS.map(item => ({ id: item.id, label: item.label, provider: item.provider, envKey: item.envKey, configured: Boolean(routeCredential(item.id)) }));
}
