...
const ROUTE_QUALITIES = Object.freeze(['480p', '720p', '1080p']);
...
// The public product still exposes one Seedance 2.0 model.  These route IDs
// split its upstream pools by input shape so a provider that only supports
// image-to-video can never be selected for a text-only request.
export const SEEDANCE_ROUTE_MODEL_IDS = Object.freeze({
  TEXT: 'seedance-2.0-text',
  IMAGE: 'seedance-2.0-img',
});

const LEGACY_ROUTE_MODEL_IDS = Object.freeze({
  SEEDANCE_2: 'seedance-2.0',
});

const ROUTE_MODEL_ID_ALIASES = Object.freeze({
  'seedance2.0_text': SEEDANCE_ROUTE_MODEL_IDS.TEXT,
  'seedance2.0_img': SEEDANCE_ROUTE_MODEL_IDS.IMAGE,
  'seedance-2.0_text': SEEDANCE_ROUTE_MODEL_IDS.TEXT,
  'seedance-2.0_img': SEEDANCE_ROUTE_MODEL_IDS.IMAGE,
});
...
const platformCapabilities = Object.freeze({
  ...
  [SEEDANCE_ROUTE_MODEL_IDS.TEXT]: Object.freeze({ duration: 15, ratios: ['16:9', '9:16', '1:1'], image: 0, video: 3, audio: 3 }),
  [SEEDANCE_ROUTE_MODEL_IDS.IMAGE]: Object.freeze({ duration: 15, ratios: ['16:9', '9:16', '1:1'], image: 9, video: 3, audio: 3, minImage: 1 }),
...
function routeModelIdForRequest(logicalModelId, referenceCounts = {}) {
  const requested = normalizeRouteModelId(logicalModelId);
  if (requested === LEGACY_ROUTE_MODEL_IDS.SEEDANCE_2) {
    return Number(referenceCounts.image || 0) > 0 ? SEEDANCE_ROUTE_MODEL_IDS.IMAGE : SEEDANCE_ROUTE_MODEL_IDS.TEXT;
  }
  return requested;
}

function publicRouteModelId(logicalModelId) {
  const normalized = normalizeRouteModelId(logicalModelId);
  return [SEEDANCE_ROUTE_MODEL_IDS.TEXT, SEEDANCE_ROUTE_MODEL_IDS.IMAGE].includes(normalized)
    ? LEGACY_ROUTE_MODEL_IDS.SEEDANCE_2
    : normalized;
}
...
function rowToRoute(row) {
  if (!row) return null;
  const variantCapabilities = [SEEDANCE_ROUTE_MODEL_IDS.TEXT, SEEDANCE_ROUTE_MODEL_IDS.IMAGE].includes(normalizeRouteModelId(row.logical_model_id))
    ? platformCapabilities[normalizeRouteModelId(row.logical_model_id)]
    : null;
  const fallback = variantCapabilities || defaultsById.get(row.id)?.capabilities || platformCapabilities[row.logical_model_id] || platformCapabilities[publicRouteModelId(row.logical_model_id)] || {};
  ...
  return {
    ...
    inputMode: row.logical_model_id === SEEDANCE_ROUTE_MODEL_IDS.IMAGE ? 'IMAGE' : row.logical_model_id === SEEDANCE_ROUTE_MODEL_IDS.TEXT ? 'TEXT' : '',
...
export function ensureDefaultModelRoutes() {
  ...
  tx(() => {
    ...
    for (const logicalModelId of Object.keys(platformCapabilities)) for (const quality of ROUTE_QUALITIES.filter(quality => quality !== '1080p' || logicalModelId === 'seedance-2.5')) {
...
export function createModelRoute({ logicalModelId, quality, credentialId, upstreamModelId, priority, costYuan, salePriceYuan, adminEnabled = true, displayName = '' }, { actorUserId, audit = {} } = {}) {
  return tx(() => {
    const requestedLogicalId = normalizeRouteModelId(logicalModelId);
    // Older admin clients only know the public model ID. Preserve that API by
    // treating such additions as text routes; new clients must choose a pool.
    const logicalId = requestedLogicalId === LEGACY_ROUTE_MODEL_IDS.SEEDANCE_2
      ? SEEDANCE_ROUTE_MODEL_IDS.TEXT
      : requestedLogicalId;
    if (!platformCapabilities[logicalId]) throw Object.assign(new Error('模型类型不支持新增调用线路，请选择文生或图生线路池'), { statusCode: 400 });
    const normalizedQuality = String(quality || '');
    if (!ROUTE_QUALITIES.includes(normalizedQuality) || (normalizedQuality === '1080p' && logicalId !== 'seedance-2.5')) throw Object.assign(new Error('分辨率无效'), { statusCode: 400 });
...
export function selectModelRoute({ logicalModelId, quality, duration, aspectRatio, referenceCounts = {} }) {
  const requestedId = normalizeRouteModelId(logicalModelId);
  const routeModelId = routeModelIdForRequest(requestedId, referenceCounts);
  const allRoutes = listModelRoutes();
  const directRoutes = allRoutes.filter(item => item.logicalModelId === routeModelId && item.quality === quality);
  const legacyRoutes = requestedId === LEGACY_ROUTE_MODEL_IDS.SEEDANCE_2
    ? allRoutes.filter(item => item.logicalModelId === LEGACY_ROUTE_MODEL_IDS.SEEDANCE_2 && item.quality === quality)
    : [];
  const routes = directRoutes.length ? directRoutes : legacyRoutes;
  const policy = directRoutes.length
    ? routePolicy(routeModelId, quality)
    : routePolicy(requestedId, quality) || routePolicy(routeModelId, quality);
...
export function availableModelRouteQualities(logicalModelId, { referenceCounts = {}, aspectRatios = ['16:9', '9:16', '1:1'] } = {}) {
  ...
  return ROUTE_QUALITIES.filter(quality => aspectRatios.some(aspectRatio =>
    selectModelRoute({ logicalModelId, quality, duration, aspectRatio, referenceCounts })));
}
...
export function publicModelPrices() {
  const publicModelIds = Object.keys(platformCapabilities).filter(item => ![SEEDANCE_ROUTE_MODEL_IDS.TEXT, SEEDANCE_ROUTE_MODEL_IDS.IMAGE].includes(item));
  return publicModelIds.flatMap(logicalModelId => ROUTE_QUALITIES.filter(quality => quality !== '1080p' || logicalModelId === 'seedance-2.5').map(quality => {
    // Public pricing intentionally always comes from the text pool for
    // Seedance 2.0. The image pool may have a different backend price, but
    // users must see one stable model price; submission still charges the
    // selected route's configured price.
    const pricingModelId = logicalModelId === LEGACY_ROUTE_MODEL_IDS.SEEDANCE_2 ? SEEDANCE_ROUTE_MODEL_IDS.TEXT : logicalModelId;
    const route = selectModelRoute({ logicalModelId, quality, duration: platformCapabilities[pricingModelId].duration, aspectRatio: '16:9', referenceCounts: {} });
    ...
    return { modelId: logicalModelId, label, quality, duration: platformCapabilities[pricingModelId].duration, available: Boolean(route), credits: route?.salePriceCredits ?? null, yuan: route?.salePriceYuan ?? null, selectedRouteId: route?.id || '', selectedRouteName: route?.displayName || '', priceVersion: publicRoutePriceVersion(route) };
  }));
}
...
export function updateModelRoute(id, patch, { actorUserId, expectedVersion = null, audit = {} } = {}) {
  return tx(() => {
    ...
    if (!before) throw Object.assign(new Error('调用线路不存在'), { statusCode: 404 });
    if (expectedVersion !== null && Number(expectedVersion) !== before.version) throw Object.assign(new Error('线路配置已被其他管理员修改，请刷新后重试'), { statusCode: 409 });
    const requestedLogicalId = patch.logicalModelId === undefined ? before.logicalModelId : normalizeRouteModelId(patch.logicalModelId);
    const logicalModelId = patch.logicalModelId === undefined
      ? requestedLogicalId
      : requestedLogicalId === LEGACY_ROUTE_MODEL_IDS.SEEDANCE_2 ? SEEDANCE_ROUTE_MODEL_IDS.TEXT : requestedLogicalId;
    if (!platformCapabilities[logicalModelId]) throw Object.assign(new Error('模型类型不支持，请选择文生或图生线路池'), { statusCode: 400 });
