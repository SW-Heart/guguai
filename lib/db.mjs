...
const DEFAULT_MODEL_CONTROLS = [
  ...
  ['grok', 'video', 1, 1, 10],
  ['grok-15', 'video', 1, 0, 20],
  ['veo', 'video', 1, 0, 30],
  ['oai', 'video', 1, 1, 40],
  ['veo-31', 'video', 1, 1, 50],
  ['minimax-h3', 'video', 1, 0, 60],
  ['seedance-2.0', 'video', 1, 0, 70],
];
...
function seedDefaults(handle) {
  ...
  // Keep launch-gated video model rows visible but disabled until their launch. Explicit admin changes are preserved.
  handle.prepare(`
    UPDATE model_controls
    SET user_visible = 1, enabled = 0, updated_at = :updatedAt
    WHERE model_id IN ('grok-15', 'veo') AND updated_by IS NULL AND version = 1
