...
/** Rejects every pre-baseline database before opening it for writes. */
function assertSupportedDatabase(target) {
  if (target === ':memory:' || !existsSync(target) || statSync(target).size === 0) return;
  ...
  try {
    ...
    if (!row || row.value !== String(SCHEMA_VERSION)) {
      ...
      throw new Error(`数据库包含旧 schema_version=${version}；本版本只接受全新基线 v${SCHEMA_VERSION}，不支持迁移，请使用空 DATA_DIR 启动`);
    }
    ...
    if (baseline?.value !== SCHEMA_BASELINE) {
      throw new Error(`数据库 schema_version=${SCHEMA_VERSION} 不是 R2-only 全新基线；本版本不支持迁移，请使用空 DATA_DIR 启动`);
    }
  } catch (error) {
    if (String(error?.message || '').includes('请使用空 DATA_DIR')) throw error;
    throw new Error(`无法验证现有数据库；本版本不支持迁移，请使用空 DATA_DIR 启动：${error.message}`, { cause: error });
  } finally {
    probe?.close();
  }
}
...
export function openDatabase({ env = process.env, file = null, verbose = false } = {}) {
  ...
  assertSupportedDatabase(target);
...
function applyBaselineSchema(handle) {
  handle.exec('BEGIN');
  try {
