import { AGENT_SCHEMA_SQL } from '../repositories/agent-sessions.mjs';
...
export const SCHEMA_MIN_ROLLBACK_VERSION = 1;
...
const DEFAULT_MODEL_CONTROLS = [
  ...
  ['gpt-image-2.5', 'image', 1, 1, 1],
  ['midjourney', 'image', 1, 1, 2],
  ['grok', 'video', 1, 1, 60],
  ['minimax-h3-15s', 'video', 1, 1, 30],
  ['motion-retargeting', 'video', 0, 1, 70],
  ['seedance-2.0', 'video', 1, 1, 10],
  ['seedance-2.5', 'video', 1, 1, 20],
  ['seedance-2.0-fast', 'video', 0, 0, 50],
  ['minimax-h3', 'video', 0, 0, 60],
  ['oai', 'video', 1, 1, 40],
  ['veo-31', 'video', 1, 1, 50],
  ['veo', 'video', 1, 0, 90],
];
...
CREATE TABLE IF NOT EXISTS schema_migrations (
  ...
  rollback_version INTEGER NOT NULL
);
...
CREATE TABLE IF NOT EXISTS generation_jobs (
  ...
  CHECK (state IN ('pending', 'leased', 'done', 'manual_review')),
  CHECK (attempt_count >= 0),
  CHECK (lease_token >= 0)
);
CREATE INDEX IF NOT EXISTS idx_generation_jobs_due
  ON generation_jobs(state, next_run_at);
CREATE INDEX IF NOT EXISTS idx_generation_jobs_lease
...
CREATE TABLE IF NOT EXISTS generation_requests (
  ...
  generation_ids_json TEXT NOT NULL,
  created_at        INTEGER NOT NULL,
  updated_at        INTEGER NOT NULL,
  PRIMARY KEY (user_id, idempotency_key)
);
CREATE INDEX IF NOT EXISTS idx_generation_requests_time
...
const SCHEMA_MIGRATIONS = Object.freeze([
  Object.freeze({ version:1, name:'r2-only-v1', rollbackVersion:SCHEMA_MIN_ROLLBACK_VERSION }),
]);
const schemaChecksum = createHash('sha256').update(SCHEMA_SQL).digest('hex');

function applyMigrationMetadata(handle) {
  const expectedVersions = new Set(SCHEMA_MIGRATIONS.map(migration => migration.version));
  const existing = handle.prepare('SELECT version, name, checksum, rollback_version AS rollbackVersion FROM schema_migrations ORDER BY version').all();
  for (const row of existing) {
    if (!expectedVersions.has(Number(row.version))) throw new Error(`数据库包含不支持的迁移版本 ${row.version}`);
    const migration = SCHEMA_MIGRATIONS.find(item => item.version === Number(row.version));
    if (row.name !== migration.name || row.checksum !== schemaChecksum || Number(row.rollbackVersion) !== migration.rollbackVersion) {
      throw new Error(`数据库迁移记录校验失败：版本 ${row.version}`);
    }
  }
  for (const migration of SCHEMA_MIGRATIONS) {
    const appliedAt = new Date().toISOString();
    handle.prepare(`
      INSERT INTO schema_migrations(version, name, checksum, applied_at, rollback_version)
      VALUES(:version, :name, :checksum, :appliedAt, :rollbackVersion)
      ON CONFLICT(version) DO NOTHING`).run({ version:migration.version, name:migration.name, checksum:schemaChecksum, appliedAt, rollbackVersion:migration.rollbackVersion });
  }
  handle.prepare(`
    INSERT INTO schema_meta(key, value) VALUES('schema_min_rollback_version', :version)
    ON CONFLICT(key) DO UPDATE SET value = excluded.value`).run({ version:String(SCHEMA_MIN_ROLLBACK_VERSION) });
  handle.prepare(`
    INSERT INTO schema_meta(key, value) VALUES('schema_checksum', :checksum)
    ON CONFLICT(key) DO UPDATE SET value = excluded.value`).run({ checksum:schemaChecksum });
}
...
function ensureModelPricing(handle) {
  if (!hasColumn(handle, 'pricing_versions', 'model_prices_json')) {
    handle.exec("ALTER TABLE pricing_versions ADD COLUMN model_prices_json TEXT NOT NULL DEFAULT '{}'");
  }
}

function ensureDramaProjectRevision(handle) {
  if (!hasColumn(handle, 'drama_projects', 'revision')) {
    handle.exec('ALTER TABLE drama_projects ADD COLUMN revision INTEGER NOT NULL DEFAULT 1');
  }
  const updateRevision = handle.prepare(`
    UPDATE drama_projects SET revision = :revision WHERE id = :id
  `);
  const updateDocument = handle.prepare(`
    UPDATE drama_projects
    SET revision = :revision,
        doc_json = :docJson
    WHERE id = :id
  `);
  for (const row of handle.prepare('SELECT id, revision, doc_json FROM drama_projects').all()) {
    let document;
    try { document = JSON.parse(row.doc_json); } catch {
      updateRevision.run({ id: row.id, revision: Math.max(1, Number(row.revision) || 1) });
      continue;
    }
    const revision = Math.max(1, Number(document.revision) || Number(row.revision) || 1);
    if (Number(row.revision) === revision && Number(document.revision) === revision) continue;
    document.revision = revision;
    updateDocument.run({ id: row.id, revision, docJson: JSON.stringify(document) });
  }
}
...
export function openDatabase({ env = process.env, file = null, verbose = false } = {}) {
  ...
  try {
    ...
    ensureSeedance1080pRoutes(handle);
    db = handle;
    if (verbose) {
...
function ensureSeedance1080pRoutes(handle) {
  const tables = ['model_routes', 'model_route_policies'];
  const definitions = tables.map(name => handle.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name=?").get(name)?.sql);
  if (definitions.every(definition => definition?.includes("'1080p'"))) return;
  if (definitions.some(definition => !definition?.includes("CHECK (quality IN ('480p', '720p'))"))) throw new Error('模型线路表结构无法升级');
  handle.exec('PRAGMA foreign_keys = OFF');
  try {
    handle.exec('BEGIN');
    for (const [index, name] of tables.entries()) {
      const replacement = `${name}_1080p_upgrade`;
      const definition = definitions[index]
        .replace(`CREATE TABLE ${name}`, `CREATE TABLE ${replacement}`)
        .replace("CHECK (quality IN ('480p', '720p'))", "CHECK (quality IN ('480p', '720p', '1080p'))");
      handle.exec(definition);
      handle.exec(`INSERT INTO ${replacement} SELECT * FROM ${name}`);
    }
    handle.exec('DROP TABLE model_route_policies');
    handle.exec('DROP TABLE model_routes');
    for (const name of tables) handle.exec(`ALTER TABLE ${name}_1080p_upgrade RENAME TO ${name}`);
    handle.exec('CREATE INDEX idx_model_routes_selection ON model_routes(logical_model_id, quality, admin_enabled, priority, id)');
    const violations = handle.prepare('PRAGMA foreign_key_check').all();
    if (violations.length) throw new Error('模型线路表升级后外键校验失败');
    handle.exec('COMMIT');
  } catch (error) {
...
function applyBaselineSchema(handle, { migrateV12 = false } = {}) {
  ...
  try {
    ...
    handle.exec(AGENT_SCHEMA_SQL);
    // Feature tables must not alter the already deployed v1 baseline checksum.
    handle.exec(`CREATE TABLE IF NOT EXISTS deleted_model_routes (
  id TEXT PRIMARY KEY,
  deleted_at TEXT NOT NULL
);`);
    // Additive feature extension: keep the immutable baseline checksum unchanged.
    handle.exec(`CREATE TABLE IF NOT EXISTS viral_projects (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  device_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_viral_projects_scope ON viral_projects(user_id, device_id, workspace_id, updated_at DESC);

`);
    handle.prepare("INSERT OR IGNORE INTO schema_meta(key,value) VALUES('viral_projects_schema_version','1')").run();
    ensureModelPricing(handle);
    ensureDramaProjectRevision(handle);
    applyMigrationMetadata(handle);
    handle.exec(`
