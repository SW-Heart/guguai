...
function migrateV4ToV5(handle) {
  const legacyModelId = 'grok-15';
  const currentModelId = 'minimax-h3-15s';
  const legacyGrokModelId = 'legacy-grok-video-1.5';

  // Rename the model-control row while preserving any administrator override.
  handle.exec(`
    INSERT INTO model_controls(model_id, kind, user_visible, enabled, sort_order, version, updated_by, updated_at)
    SELECT '${currentModelId}', kind, user_visible, enabled, sort_order, version, updated_by, updated_at
    FROM model_controls
    WHERE model_id = '${legacyModelId}'
      AND NOT EXISTS (SELECT 1 FROM model_controls WHERE model_id = '${currentModelId}');
    DELETE FROM model_controls WHERE model_id = '${legacyModelId}';
  `);

  // AutoDL rows represented GuGu 2.0. Older non-AutoDL rows used the same
  // accidental ID for the former Grok route, so give those rows an explicit
  // historical ID instead of mislabeling them as MiniMax H3.
  handle.prepare(`
    UPDATE generations
    SET model_id = :currentModelId
    WHERE model_id = :legacyModelId AND provider = 'autodl'
  `).run({ currentModelId, legacyModelId });
  handle.prepare(`
    UPDATE generations
    SET model_id = :legacyGrokModelId
    WHERE model_id = :legacyModelId AND provider IS NOT NULL AND provider <> 'autodl'
  `).run({ legacyGrokModelId, legacyModelId });
  const generationRows = handle.prepare('SELECT id, provider, doc_json FROM generations').all();
  const updateGeneration = handle.prepare('UPDATE generations SET doc_json = :docJson WHERE id = :id');
  for (const row of generationRows) {
    let task;
    try { task = JSON.parse(row.doc_json); } catch { continue; }
    let changed = false;
    if (row.provider === 'autodl' && (task.videoModelId === legacyModelId || task.videoProfile === 'gugu-1.0')) {
      task.videoModelId = currentModelId;
      task.videoProfile = currentModelId;
      changed = true;
    } else if (row.provider && row.provider !== 'autodl' && task.videoModelId === legacyModelId) {
      task.videoModelId = legacyGrokModelId;
      task.videoProfile = legacyGrokModelId;
      changed = true;
    }
    if (changed) updateGeneration.run({ id: row.id, docJson: JSON.stringify(task) });
  }

  // Existing professional projects can also hold the old model ID in shots.
  const projectRows = handle.prepare('SELECT id, doc_json FROM drama_projects').all();
  const updateProject = handle.prepare('UPDATE drama_projects SET doc_json = :docJson WHERE id = :id');
  for (const row of projectRows) {
    let project;
    try { project = JSON.parse(row.doc_json); } catch { continue; }
    let changed = false;
    for (const shot of Array.isArray(project.shots) ? project.shots : []) {
      if (shot.generation?.modelId === legacyModelId) {
        shot.generation.modelId = currentModelId;
        changed = true;
      }
    }
    if (changed) updateProject.run({ id: row.id, docJson: JSON.stringify(project) });
  }

  // Model-control audit targets should continue resolving to the renamed row.
  handle.prepare(`
    UPDATE audit_events
    SET target_id = :currentModelId
    WHERE target_type = 'model' AND target_id = :legacyModelId
  `).run({ currentModelId, legacyModelId });
}

function migrateV5ToV6() {
  // Route tables are created by the idempotent schema block below. Keeping the
  // version step explicit preserves verified pre-upgrade backups.
}

function migrateV6ToV7(handle) {
  addColumnIfMissing(handle, 'model_routes', 'sale_price_fen', 'INTEGER');
}

function migrateV7ToV8() {
  // Announcement tables are created by the idempotent schema block above.
  // Keeping an explicit version step preserves the upgrade/backup contract.
}

function migrateV8ToV9(handle) {
  addColumnIfMissing(handle, 'users', 'phone_number', 'TEXT');
}

function migrateV9ToV10(handle) {
  addColumnIfMissing(handle, 'users', 'nickname', 'TEXT');
}

function migrateV10ToV11() {
  // Cross-column login-name triggers are created by the idempotent schema
  // block below. Keeping the version step explicit preserves upgrade backups.
}

function migrateV11ToV12() {
  // 支付订单、退款和通知幂等表由上方幂等 schema block 创建。
}
...
function applySchema(handle) {
  ...
  try {
    ...
    if (current < 5 && hasMeta) migrateV4ToV5(handle);
    if (current < 6 && hasMeta) migrateV5ToV6(handle);
    if (current < 7 && hasMeta) migrateV6ToV7(handle);
    if (current < 8 && hasMeta) migrateV7ToV8(handle);
    if (current < 9) migrateV8ToV9(handle);
    if (current < 10) migrateV9ToV10(handle);
    if (current < 11) migrateV10ToV11(handle);
    if (current < 12) migrateV11ToV12(handle);
    handle.exec(`
