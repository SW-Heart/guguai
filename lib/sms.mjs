...
export function smsConfigFromEnv(env = process.env) {
  const accessKeyId = stringValue(env.SMS_ACCESS_KEY_ID || env.ALIBABA_CLOUD_ACCESS_KEY_ID || env.ALIYUN_ACCESS_KEY_ID);
  const accessKeySecret = stringValue(env.SMS_ACCESS_KEY_SECRET || env.ALIBABA_CLOUD_ACCESS_KEY_SECRET || env.ALIYUN_ACCESS_KEY_SECRET);
