...
function totalSpentMicro(userId) {
  return sql(`SELECT COALESCE(SUM(-amount_micro), 0) AS total_spent_micro
              FROM credit_entries
              WHERE user_id = :userId AND amount_micro < 0`).get({ userId }).total_spent_micro;
}
...
function listUsers(url) {
  ...
  const page = pageQuery({ scope: 'admin-users', table: 'users', where, params, limit: parseLimit(url.searchParams.get('limit')), cursor: url.searchParams.get('cursor'), select: `id, username, role, status, credit_balance_micro, credit_held_micro,
    (SELECT COALESCE(SUM(-amount_micro), 0) FROM credit_entries WHERE credit_entries.user_id = users.id AND amount_micro < 0) AS total_spent_micro,
