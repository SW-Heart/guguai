...
import { checkModelRoutes, createModelRoute, createModelRouteCredential, listModelRouteChannels, listModelRoutes, publicModelPrices, routePolicy, updateModelRoute, updateModelRouteCredential, updateRoutePolicy } from './model-routes.mjs';
...
function listClientLogs(url) {
  ...
  if (url.searchParams.get('userId')) { where.push('user_id = :userId'); params.userId = url.searchParams.get('userId'); }
  addTimeRange(url, where, params);
  const page = pageQuery({ scope: 'admin-client-log', table: 'system_events', where, params, limit: parseLimit(url.searchParams.get('limit')), cursor: url.searchParams.get('cursor'), select: 'id, user_id, message, details_json, created_at' });
  return {
...
async function handleAuthorized(req, res, url, session, id) {
  ...
  if (url.pathname === '/api/admin/model-routes' && req.method === 'GET') {
    ...
    const policies = [...new Map(items.map(item => [`${item.logicalModelId}:${item.quality}`, routePolicy(item.logicalModelId, item.quality)])).values()];
