...
import { createPricingVersion, currentPricing, pricingHistory, parsePrice, modelPriceFields } from './pricing.mjs';
import { listModelControls, modelControl, updateModelControl } from './model-controls.mjs';
import { checkModelRoutes, deleteModelRoute, createModelRoute, createModelRouteCredential, listModelRouteChannels, listModelRoutes, listRoutePolicies, publicModelPrices, updateModelRoute, updateModelRouteCredential, updateRoutePolicy } from './model-routes.mjs';
import { decodeCursor, encodeCursor, parseLimit } from './store.mjs';
import { createAnnouncement, listAdminAnnouncements, updateAnnouncement } from './notifications.mjs';
import { isSupportLogKey, signedSupportLogUrl } from './support-logs.mjs';
...
const totalSpentExpression = `COALESCE(SUM(CASE
  WHEN amount_micro < 0 THEN -amount_micro
  WHEN type = 'generation_refund' THEN -amount_micro
  ELSE 0
END), 0)`;
...
function totalSpentMicro(userId) {
  return sql(`SELECT ${totalSpentExpression} AS total_spent_micro
              FROM credit_entries
              WHERE user_id = :userId`).get({ userId }).total_spent_micro;
}
...
function userRow(row) {
  const spentMicro = row.total_spent_micro ?? totalSpentMicro(row.id);
  return {
    id: row.id, username: row.username, phoneNumber: row.phone_number || null, nickname: row.nickname || null, role: row.role, status: row.status,
    credits: microToCredits(row.credit_balance_micro), held: microToCredits(row.credit_held_micro),
    totalSpent: microToCredits(spentMicro),
...
function listUsers(url) {
  ...
  const page = pageQuery({ scope: 'admin-users', table: 'users', where, params, limit: parseLimit(url.searchParams.get('limit')), cursor: url.searchParams.get('cursor'), select: `id, username, role, status, credit_balance_micro, credit_held_micro,
    phone_number, nickname,
    (SELECT ${totalSpentExpression} FROM credit_entries WHERE credit_entries.user_id = users.id) AS total_spent_micro,
    created_at, updated_at, disabled_at, admin_note` });
  return { items: page.rows.map(userRow), total: page.total, nextCursor: page.nextCursor };
}

function listPaidOrders(url) {
  const where = ["o.status IN ('PAID', 'PARTIALLY_REFUNDED', 'REFUNDED')", 'o.paid_at IS NOT NULL'];
  const params = {};
  const query = String(url.searchParams.get('query') || '').trim();
  const from = String(url.searchParams.get('from') || '').trim();
  const to = String(url.searchParams.get('to') || '').trim();
  if (query) {
    where.push('(u.username LIKE :query OR u.id = :exactQuery OR o.out_trade_no LIKE :query OR o.alipay_trade_no LIKE :query)');
    params.query = `%${query}%`;
    params.exactQuery = query;
  }
  if (from) { where.push('o.paid_at >= :from'); params.from = from; }
  if (to) { where.push('o.paid_at < :to'); params.to = to; }

  const conditions = [...where];
  const limit = parseLimit(url.searchParams.get('limit'));
  const queryParams = { ...params, limit: limit + 1 };
  const position = decodeCursor('admin-paid-orders', url.searchParams.get('cursor'));
  if (position) {
    conditions.push('(o.paid_at < :cursorTime OR (o.paid_at = :cursorTime AND o.out_trade_no > :cursorId))');
    queryParams.cursorTime = position.t;
    queryParams.cursorId = position.i;
  }
  ...
  const summary = sql(`
    SELECT COUNT(*) AS total,
           ...
           COALESCE(SUM(o.refunded_amount_fen), 0) AS refunded_amount_fen
    FROM alipay_payment_orders o JOIN users u ON u.id = o.user_id
    WHERE ${whereSql}
  `).get(params);
  const rows = sql(`
    SELECT o.out_trade_no, o.alipay_trade_no, o.user_id, u.username, o.status, o.subject,
           o.total_amount_fen, o.refunded_amount_fen, o.credits_micro, o.paid_at, o.created_at
    FROM alipay_payment_orders o JOIN users u ON u.id = o.user_id
    WHERE ${conditions.join(' AND ')}
    ORDER BY o.paid_at DESC, o.out_trade_no ASC
    LIMIT :limit
  `).all(queryParams);
  const hasMore = rows.length > limit;
  const items = hasMore ? rows.slice(0, limit) : rows;
  ...
  return {
    items: items.map(row => ({
      orderNo: row.out_trade_no,
      tradeNo: row.alipay_trade_no || null,
      userId: row.user_id,
      username: row.username,
      status: row.status,
      subject: row.subject,
      amount: row.total_amount_fen / 100,
      amountFen: row.total_amount_fen,
      refundedAmount: row.refunded_amount_fen / 100,
      refundedAmountFen: row.refunded_amount_fen,
      netAmount: (row.total_amount_fen - row.refunded_amount_fen) / 100,
    ...
    summary: {
      ...
      totalAmount: summary.total_amount_fen / 100,
      refundedAmount: summary.refunded_amount_fen / 100,
      netAmount: (summary.total_amount_fen - summary.refunded_amount_fen) / 100,
    },
    nextCursor: hasMore && last ? encodeCursor('admin-paid-orders', { t: last.paid_at, i: last.out_trade_no }) : null,
  };
}
...
function userNicknameColumn(table, userColumn = 'user_id', alias = 'user_nickname') {
  return `(SELECT COALESCE(NULLIF(nickname, ''), NULLIF(username, ''), NULLIF(phone_number, '')) FROM users WHERE users.id = ${table}.${userColumn}) AS ${alias}`;
}

function listGenerations(url) {
  const where = [];
  const params = {};
  const taskId = String(url.searchParams.get('taskId') || url.searchParams.get('generationId') || '').trim();
  const userId = url.searchParams.get('userId');
  const modelId = url.searchParams.get('modelId');
  const status = url.searchParams.get('status');
  const type = url.searchParams.get('type');
  addTimeRange(url, where, params);
  if (taskId) { where.push('id = :taskId'); params.taskId = taskId; }
  ...
  const page = pageQuery({ scope: 'admin-generations', table: 'generations', where, params, limit: parseLimit(url.searchParams.get('limit')), cursor: url.searchParams.get('cursor'), select: `id, user_id, ${userNicknameColumn('generations')}, type, status, credit_cost_micro, credit_status, pricing_version, model_id, provider, asset_id, created_at, updated_at, doc_json` });
  return { items: page.rows.map(row => ({ id: row.id, userId: row.user_id, userNickname: row.user_nickname || '', type: row.type, status: row.status, creditCost: row.credit_cost_micro === null ? Number(row.credit_cost || 0) : microToCredits(row.credit_cost_micro), creditStatus: row.credit_status, pricingVersion: row.pricing_version, modelId: row.model_id, provider: row.provider, assetId: row.asset_id, createdAt: row.created_at, updatedAt: row.updated_at, details: rowDoc(row) })), total: page.total, nextCursor: page.nextCursor };
}
...
function listCredits(url) {
  ...
  const params = {};
  const taskId = String(url.searchParams.get('taskId') || url.searchParams.get('generationId') || '').trim();
  if (taskId) { where.push('generation_id = :taskId'); params.taskId = taskId; }
  ...
  addTimeRange(url, where, params);
  const page = pageQuery({ scope: 'admin-credits', table: 'credit_entries', where, params, limit: parseLimit(url.searchParams.get('limit')), cursor: url.searchParams.get('cursor'), select: `id, user_id, ${userNicknameColumn('credit_entries')}, actor_user_id, type, reason_code, note, amount_micro, balance_after_micro, generation_id, request_id, created_at, doc_json` });
  return { items: page.rows.map(row => ({ id: row.id, userId: row.user_id, userNickname: row.user_nickname || '', actorUserId: row.actor_user_id, type: row.type, reasonCode: row.reason_code, note: row.note, amount: microToCredits(Math.abs(row.amount_micro)) * (row.amount_micro < 0 ? -1 : 1), balanceAfter: microToCredits(row.balance_after_micro), generationId: row.generation_id, requestId: row.request_id, createdAt: row.created_at, details: rowDoc(row) })), total: page.total, nextCursor: page.nextCursor };
}
...
function listLlmUsage(url) {
  ...
  addTimeRange(url, where, params);
  const page = pageQuery({ scope: 'admin-llm', table: 'llm_usage', where, params, limit: parseLimit(url.searchParams.get('limit')), cursor: url.searchParams.get('cursor'), select: `id, user_id, ${userNicknameColumn('llm_usage')}, status, model, input_tokens, output_tokens, charged_micro, created_at, doc_json` });
  return { items: page.rows.map(row => ({ id: row.id, userId: row.user_id, userNickname: row.user_nickname || '', status: row.status, modelId: row.model, inputTokens: row.input_tokens, outputTokens: row.output_tokens, charged: row.charged_micro === null ? null : microToCredits(row.charged_micro), createdAt: row.created_at, details: rowDoc(row) })), total: page.total, nextCursor: page.nextCursor };
}
...
function listAudit(url) {
  ...
  addTimeRange(url, where, params);
  const page = pageQuery({ scope: 'admin-audit', table: 'audit_events', where, params, limit: parseLimit(url.searchParams.get('limit')), cursor: url.searchParams.get('cursor'), select: `id, actor_user_id, ${userNicknameColumn('audit_events', 'actor_user_id', 'actor_nickname')}, action, target_type, target_id, request_id, status, before_json, after_json, metadata_json, created_at` });
  return { items: page.rows.map(row => ({ id: row.id, actorUserId: row.actor_user_id, actorNickname: row.actor_nickname || '', action: row.action, targetType: row.target_type, targetId: row.target_id, requestId: row.request_id, status: row.status, before: rowDoc({ doc_json: row.before_json }), after: rowDoc({ doc_json: row.after_json }), metadata: rowDoc({ doc_json: row.metadata_json }), createdAt: row.created_at })), total: page.total, nextCursor: page.nextCursor };
}
...
function listSystem(url) {
  ...
  addTimeRange(url, where, params);
  const page = pageQuery({ scope: 'admin-system', table: 'system_events', where, params, limit: parseLimit(url.searchParams.get('limit')), cursor: url.searchParams.get('cursor'), select: `id, level, category, request_id, user_id, ${userNicknameColumn('system_events')}, model_id, generation_id, message, details_json, created_at` });
  return { items: page.rows.map(row => ({ id: row.id, level: row.level, category: row.category, requestId: row.request_id, userId: row.user_id, userNickname: row.user_nickname || '', modelId: row.model_id, generationId: row.generation_id, message: row.message, details: rowDoc({ doc_json: row.details_json }), createdAt: row.created_at })), total: page.total, nextCursor: page.nextCursor };
}

// 客户端上传的诊断日志包共用 system_events 表，用 category 区分。日志包本身
// 在对象存储里，这里只列出元数据，下载走单独的签名跳转接口。
function listClientLogs(url) {
  const where = ["category = 'client_log'"];
  const params = {};
  if (url.searchParams.get('userId')) { where.push('user_id = :userId'); params.userId = url.searchParams.get('userId'); }
  addTimeRange(url, where, params);
  const page = pageQuery({ scope: 'admin-client-log', table: 'system_events', where, params, limit: parseLimit(url.searchParams.get('limit')), cursor: url.searchParams.get('cursor'), select: `id, user_id, ${userNicknameColumn('system_events')}, message, details_json, created_at` });
  return {
    items: page.rows.map(row => {
      const details = rowDoc({ doc_json: row.details_json }) || {};
      return {
        id: row.id,
        ...
        downloadable: Boolean(details.objectKey),
        details,
        createdAt: row.created_at,
      };
    }),
...
function overview(url) {
  ...
  const spent = sql(`SELECT ${totalSpentExpression} AS spent FROM credit_entries WHERE created_at >= COALESCE(:from, '0000') AND created_at < COALESCE(:to, '9999')`).get({ from: from || null, to: to || null });
  return {
...
async function handleAuthorized(req, res, url, session, id) {
  ...
  if (url.pathname === '/api/admin/auth/session' && req.method === 'GET') {
    const csrf = randomToken();
    sql('UPDATE sessions SET csrf_token_hash = :csrfHash WHERE token_hash = :tokenHash').run({ csrfHash: hashToken(csrf), tokenHash: session.tokenHash });
    return sendJson(res, 200, { admin: session.user, csrfToken: csrf });
  }
  ...
  if (url.pathname === '/api/admin/payment-orders' && req.method === 'GET') return sendJson(res, 200, listPaidOrders(url));
  ...
  if (url.pathname === '/api/admin/model-route-credentials' && req.method === 'GET') {
    const items = listModelRouteChannels().map(item => ({ ...item, routeCount: sql('SELECT COUNT(*) AS count FROM model_routes WHERE credential_id = :id').get({ id: item.id }).count }));
    return sendJson(res, 200, { items });
  }
  if (url.pathname === '/api/admin/model-route-credentials' && req.method === 'POST') {
    const input = await bodyJson(req);
    const credential = createModelRouteCredential(input, { actorUserId: session.user.id, audit: requestAudit(req, id) });
    return sendJson(res, 201, { credential });
  }
  const credentialMatch = url.pathname.match(/^\/api\/admin\/model-route-credentials\/([\w-]+)$/);
  if (credentialMatch && req.method === 'PATCH') {
    const input = await bodyJson(req);
    const credential = updateModelRouteCredential(credentialMatch[1], input, { actorUserId: session.user.id, expectedVersion: input.expectedVersion, audit: requestAudit(req, id) });
    return sendJson(res, 200, { credential });
  }
  if (url.pathname === '/api/admin/model-route-credentials/check' && req.method === 'POST') {
    const input = await bodyJson(req);
    const credentialId = String(input.credentialId || '').trim();
    if (!credentialId) return sendJson(res, 400, { error: '渠道 Key 不能为空' });
    const routeIds = sql('SELECT id FROM model_routes WHERE credential_id = :credentialId').all({ credentialId }).map(row => row.id);
    const items = await checkModelRoutes({ routeIds });
    appendAuditEvent({ actorUserId: session.user.id, action: 'model_route_credential.check', targetType: 'model_route_credential', targetId: credentialId, after: { checked: items.length }, ...requestAudit(req, id) });
    return sendJson(res, 200, { items });
  }

  if (url.pathname === '/api/admin/model-routes' && req.method === 'GET') {
    const items = listModelRoutes();
    const policies = listRoutePolicies();
    return sendJson(res, 200, { items, policies, prices: publicModelPrices(), channels: listModelRouteChannels() });
  }
  if (url.pathname === '/api/admin/model-routes' && req.method === 'POST') {
    const input = await bodyJson(req);
    const route = createModelRoute({
      ...
      salePriceYuan: input.salePriceYuan,
      adminEnabled: input.adminEnabled,
      displayName: input.displayName,
    }, { actorUserId: session.user.id, audit: requestAudit(req, id) });
    return sendJson(res, 201, { route });
  }
  const routeMatch = url.pathname.match(/^\/api\/admin\/model-routes\/([\w-]+)$/);
  if (routeMatch && req.method === 'DELETE') {
    const input = await bodyJson(req);
    const route = deleteModelRoute(routeMatch[1], { actorUserId: session.user.id, expectedVersion: input.expectedVersion, audit: requestAudit(req, id) });
    return sendJson(res, 200, { route });
  }
  if (routeMatch && req.method === 'PATCH') {
    const input = await bodyJson(req);
    const route = updateModelRoute(routeMatch[1], input, { actorUserId: session.user.id, expectedVersion: input.expectedVersion, audit: requestAudit(req, id) });
    return sendJson(res, 200, { route });
  }
  if (url.pathname === '/api/admin/model-routes/check' && req.method === 'POST') {
    const input = await bodyJson(req);
    const routeIds = Array.isArray(input.routeIds) ? input.routeIds.map(String) : null;
    const items = await checkModelRoutes({ routeIds });
    appendAuditEvent({ actorUserId: session.user.id, action: 'model_route.check', targetType: 'model_route', targetId: routeIds?.join(',') || 'all', after: { checked: items.length }, ...requestAudit(req, id) });
    return sendJson(res, 200, { items, prices: publicModelPrices() });
  }
  if (url.pathname === '/api/admin/model-route-policy' && req.method === 'PATCH') {
    const input = await bodyJson(req);
    const policy = updateRoutePolicy(String(input.logicalModelId || ''), String(input.quality || ''), input.forcedRouteId, { actorUserId: session.user.id, expectedVersion: input.expectedVersion, audit: requestAudit(req, id) });
    return sendJson(res, 200, { policy, prices: publicModelPrices() });
  }

  if (url.pathname === '/api/admin/pricing' && req.method === 'GET') return sendJson(res, 200, { current: currentPricing(), history: pricingHistory(), fields:modelPriceFields() });
  if (url.pathname === '/api/admin/pricing' && req.method === 'POST') {
    const input = await bodyJson(req);
    const pricing = createPricingVersion({ imagePerRequest: input.imagePerRequest, videoPerSecond: input.videoPerSecond, modelPrices:input.modelPrices, actorUserId: session.user.id, expectedVersion: input.expectedVersion, note: input.note, audit: requestAudit(req, id) });
  ...
  if (url.pathname === '/api/admin/announcements' && req.method === 'GET') {
    return sendJson(res, 200, { items: listAdminAnnouncements({ status: String(url.searchParams.get('status') || '') }) });
  }
  if (url.pathname === '/api/admin/announcements' && req.method === 'POST') {
    const input = await bodyJson(req, 1_000_000);
    const announcement = createAnnouncement({ title: input.title, content: input.content, status: input.status }, { actorUserId: session.user.id });
    appendAuditEvent({ actorUserId: session.user.id, action: 'announcement.create', targetType: 'announcement', targetId: announcement.id, after: announcement, ...requestAudit(req, id) });
    return sendJson(res, 201, { announcement });
  }
  const announcementMatch = url.pathname.match(/^\/api\/admin\/announcements\/([\w-]+)$/);
  if (announcementMatch && req.method === 'PATCH') {
    const input = await bodyJson(req, 1_000_000);
    const before = listAdminAnnouncements().find(item => item.id === announcementMatch[1]);
    const announcement = updateAnnouncement(announcementMatch[1], input, { actorUserId: session.user.id, expectedVersion: input.expectedVersion });
    appendAuditEvent({ actorUserId: session.user.id, action: 'announcement.update', targetType: 'announcement', targetId: announcement.id, before, after: announcement, ...requestAudit(req, id) });
  ...
  if (url.pathname === '/api/admin/logs/client' && req.method === 'GET') return sendJson(res, 200, listClientLogs(url));
  const clientLogDownloadMatch = url.pathname.match(/^\/api\/admin\/logs\/client\/([\w-]+)\/download$/);
  if (clientLogDownloadMatch && req.method === 'GET') {
    const row = sql("SELECT id, user_id, details_json, created_at FROM system_events WHERE id = :id AND category = 'client_log'").get({ id: clientLogDownloadMatch[1] });
    if (!row) return sendJson(res, 404, { error: '客户端日志记录不存在' });
    const objectKey = (rowDoc({ doc_json: row.details_json }) || {}).objectKey;
    if (!objectKey || !isSupportLogKey(objectKey)) return sendJson(res, 404, { error: '日志包对象缺失' });
    const location = await signedSupportLogUrl(objectKey, { fileName: `gugu-client-logs-${row.id.slice(0, 8)}.log.gz` });
    appendAuditEvent({ actorUserId: session.user.id, action: 'client_log.download', targetType: 'client_log', targetId: row.id, metadata: { userId: row.user_id }, ...requestAudit(req, id) });
