...
export function findAssetBySha256(userId, sha256, size, { requireRemote = false } = {}) {
  return parseDoc(sql(`
    ...
    WHERE user_id = :userId
      ...
      ${requireRemote ? "AND COALESCE(oss_key, '') <> ''" : ''}
...
export function expireUploadIntents(nowIso, limit = 100) {
  const rows = sql(`
    SELECT id, user_id AS userId, temporary_oss_key AS temporaryOssKey,
           final_oss_key AS finalOssKey, doc_json AS docJson
    FROM upload_intents
  ...
  return rows.map(row => {
    let provider = 'oss';
    try {
      const value = JSON.parse(row.docJson || '{}');
      if (value.storageProvider === 'r2') provider = 'r2';
    } catch {}
    const { docJson, ...intent } = row;
    return { ...intent, storageProvider: provider };
  });
}
