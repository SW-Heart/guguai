 ...
 */
function keysetPage({ table, timeColumn, userId, filters = {}, limit, cursor, scope, includeTotal = true }) {
...
export function listGenerations(userId, { type = null, limit = DEFAULT_PAGE_LIMIT, cursor = null, includeTotal = true } = {}) {
  return keysetPage({
    table: 'generations', timeColumn: 'created_at', scope: 'gen',
    userId, filters: { type }, limit, cursor, includeTotal,
  });
}
...
export function findAssetBySha256(userId, sha256, size, { requireRemote = false } = {}) {
  return parseDoc(sql(`
...
export function listAssets(userId, { kind = null, search = null, limit = DEFAULT_PAGE_LIMIT, cursor = null, includeTotal = true } = {}) {
  const searchText = String(search || '').trim();
  if (!searchText) return keysetPage({
    table: 'assets', timeColumn: 'created_at', scope: 'asset',
    userId, filters: { kind }, limit, cursor, includeTotal,
  });
  ...
  let total = null;
  if (includeTotal) total = sql(`SELECT COUNT(*) AS total FROM assets WHERE ${baseWhere.join(' AND ')}`).get({ userId, ...(kind ? { kind } : {}), search: `%${searchText}%` }).total;
  const rows = sql(`
...
export function listAssetChanges(userId, { cursor = null, limit = DEFAULT_PAGE_LIMIT } = {}) {
  ...
  const rows = sql(`
    ...
    ORDER BY seq ASC
    LIMIT :limit`).all({ userId, afterSeq, limit: limit + 1 });
  const hasMore = rows.length > limit;
  const page = hasMore ? rows.slice(0, limit) : rows;
...
export function listPendingAssetDeliveries(userId, deviceId, { limit = DEFAULT_PAGE_LIMIT } = {}) {
  ...
  const rows = sql(`
    ...
    LIMIT :limit`).all({ userId, deviceId, limit: bounded });
...
/** Resolves cloud-backed assets requested by a desktop that lost its local copy. */
export function findCloudAssets(userId, ids) {
  return findAssets(userId, ids).filter(asset => asset.objectKey);
}
