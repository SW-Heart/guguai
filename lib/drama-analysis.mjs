...
- videoPrompt：用于从关键帧生成 6 秒视频的中文提示词，描述动作顺序、表演、运镜、声音和禁止变化项。
按照戏剧信息变化切镜，不要机械地每句对白切一镜。单镜只安排一个主要动作，所有镜头固定按 6 秒制作。不要新增剧本中不存在的关键人物和剧情。`;
...
export function buildDirectorPackageRepairPrompt(originalPrompt, settings = {}, feedback = null, { round = 1, requireScript = true } = {}) {
  const payload = {
    ...
    settings:{ shotDuration:Number(settings.shotDuration)||6, aspectRatio:String(settings.aspectRatio||'9:16'), totalDuration:Number(settings.totalDuration)||0 },
...
export function buildDirectorShotRepairPrompt(pack, settings = {}, recovery = analyzeDirectorPlanRecovery(pack, settings), options = {}) {
  ...
  const payload = {
    ...
    settings:{ shotDuration:Number(settings.shotDuration)||6, aspectRatio:String(settings.aspectRatio||'9:16') },
