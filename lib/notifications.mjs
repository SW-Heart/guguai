...
export function createAnnouncement({ title, content, status = 'draft' }, { actorUserId = null, createdAt = now() } = {}) {
  ...
  const normalizedContent = normalizeText(content, '公告内容', 10_000);
  const normalizedStatus = normalizeStatus(status);
  const id = randomUUID();
  const publishedAt = normalizedStatus === 'published' ? createdAt : null;
  const doc = { id, title: normalizedTitle, content: normalizedContent, status: normalizedStatus, publishedAt, createdBy: actorUserId, updatedBy: actorUserId, createdAt, updatedAt: createdAt, version: 1 };
  tx(() => {
    sql(`INSERT INTO announcements(id, title, content, status, published_at, created_by, updated_by, created_at, updated_at, version, doc_json)
      ...
      id, title: normalizedTitle, content: normalizedContent, status: normalizedStatus, publishedAt, createdBy: actorUserId, updatedBy: actorUserId, createdAt, updatedAt: createdAt, docJson: JSON.stringify(doc),
    });
  });
  return doc;
}
...
export function updateAnnouncement(id, input = {}, { actorUserId = null, expectedVersion } = {}) {
  ...
  const content = input.content === undefined ? row.content : normalizeText(input.content, '公告内容', 10_000);
  ...
  tx(() => {
    const result = sql(`UPDATE announcements
      ...
      id: row.id, title, content, status, publishedAt, updatedBy: actorUserId, updatedAt, version, expectedVersion: row.version, docJson: JSON.stringify(doc),
    });
...
export const __test = { announcementFromRow, normalizeText, normalizeStatus, publicWhere, PUBLIC_STATUSES };
