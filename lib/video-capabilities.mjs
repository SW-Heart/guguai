...
export const VIDEO_MODEL_IDS = Object.freeze({
  ...
  MOTION_RETARGETING: 'motion-retargeting',
  VEO: 'veo',
  VEO_31: 'veo-31',
  OAI: 'oai',
...
const profiles = Object.freeze({
  ...
  motionRetargeting: Object.freeze({
    key: 'motion-retargeting',
    modelId: VIDEO_MODEL_IDS.MOTION_RETARGETING,
    provider: 'autodl-motion',
    model: autodlMotionWorkflowId,
    generationTypes: ['REFERENCE'],
    aspectRatios: ['9:16', '16:9'],
    durations: Array.from({ length: 120 }, (_, index) => index + 1),
    defaultDuration: 5,
    maxImages: 1,
    qualities: ['464*832px(竖版)', '832*464px(横版)'],
    pricing: Object.freeze({ currency: 'credit', amount: 1, unit: 'second' }),
    referenceLimits: Object.freeze({ image: 1, video: 1, audio: 0, total: 2 }),
  }),
  ...
  seedance25Text: Object.freeze({
    ...
    generationTypes: ['TEXT'], aspectRatios: ['16:9', '9:16', '1:1'], durations: [30], maxImages: 0, qualities: ['480p', '720p', '1080p'], referenceLimits: seedance25ReferenceLimits,
  }),
  seedance25Reference: Object.freeze({
    key: 'seedance-2.5-reference', modelId: VIDEO_MODEL_IDS.SEEDANCE_25, provider: 'route', model: '',
    generationTypes: ['REFERENCE'], aspectRatios: ['16:9', '9:16', '1:1'], durations: [30], maxImages: 30, qualities: ['480p', '720p', '1080p'], referenceLimits: seedance25ReferenceLimits,
  }),
...
const modelCatalog = Object.freeze([
  ...
  Object.freeze({ id: VIDEO_MODEL_IDS.MINIMAX_H3_15S, label: 'Minimax H3', description: '支持最多 9 张参考图片 + 3 段参考音频，1～15 秒视频生成', iconKey: 'minimax', profiles: Object.freeze({ TEXT: profiles.minimaxH315s, REFERENCE: profiles.minimaxH315s }) }),
  Object.freeze({ id: VIDEO_MODEL_IDS.MOTION_RETARGETING, label: '动作迁移', description: '将参考视频中的动作迁移到人物照片上', featureOnly: true, profiles: Object.freeze({ REFERENCE: profiles.motionRetargeting }) }),
  ...
  Object.freeze({ id: VIDEO_MODEL_IDS.GROK, label: 'Grok 1.5', description: '全能视频模型，支持最长30秒视频，7张参考图', profiles: Object.freeze({ TEXT: profiles.standard, REFERENCE: profiles.standard }) }),
...
export function publicVideoCapabilities() {
  return {
    models: modelCatalog.filter(model => !model.featureOnly).map(model => ({
      id: model.id,
      label: model.label,
    ...
    // Keep this compatibility block limited to public display metadata. The
    // actual provider model/workflow identifiers stay server-side.
    routing: [
      { modelId: VIDEO_MODEL_IDS.GROK, label: 'Grok 1.5', mode: 'TEXT|REFERENCE', durations: [10, 15, 20, 30], qualities: ['480p', '720p'], pricing: ttapiFastPricing },
      { modelId: VIDEO_MODEL_IDS.SEEDANCE_2, label: 'Seedance 2.0', mode: 'TEXT|REFERENCE', duration: 15, qualities: ['480p', '720p'] },
      { modelId: VIDEO_MODEL_IDS.SEEDANCE_25, label: 'Seedance 2.5', mode: 'TEXT|REFERENCE', duration: 30, qualities: ['480p', '720p', '1080p'] },
      { modelId: VIDEO_MODEL_IDS.SEEDANCE_2_FAST, label: 'Seedance 2.0 Fast', mode: 'TEXT|REFERENCE', duration: 15, qualities: ['720p'] },
      { modelId: VIDEO_MODEL_IDS.OAI, label: 'Omni Flash', mode: 'TEXT|REFERENCE', duration: 10 },
      { modelId: VIDEO_MODEL_IDS.VEO_31, label: 'Veo 3.1', mode: 'TEXT|REFERENCE|FIRST&LAST', duration: 8, qualities: ['720p', '1080p'] },
      { modelId: VIDEO_MODEL_IDS.MINIMAX_H3_15S, label: 'Minimax H3', mode: 'TEXT|REFERENCE', durations: [...profiles.minimaxH315s.durations], qualities: [...profiles.minimaxH315s.qualities], pricing: autodlMinimaxH315sPricing },
      { modelId: VIDEO_MODEL_IDS.VEO, label: 'Veo 3.1 Fast', mode: 'TEXT|REFERENCE|FIRST&LAST', duration: 8, qualities: ['720p'], availability: 'coming-soon' },
    ],
...
export function validateVideoRequest(input, referenceCount = 0) {
  ...
  const defaultDuration = [VIDEO_MODEL_IDS.VEO, VIDEO_MODEL_IDS.VEO_31].includes(modelId)
    ...
    : modelId === VIDEO_MODEL_IDS.MINIMAX_H3_15S
      ...
      : modelId === VIDEO_MODEL_IDS.SEEDANCE_2
        ...
        : modelId === VIDEO_MODEL_IDS.MOTION_RETARGETING
          ? profiles.motionRetargeting.defaultDuration
        : requestedType === 'FIRST&LAST' ? profiles.continuity.durations[0] : profiles.standard.durations[0];
  const durationInput = input.duration;
  const requestedDuration = durationInput === undefined || durationInput === null || String(durationInput).trim() === ''
    ? defaultDuration
    : Number(durationInput);
  if (!Number.isFinite(requestedDuration) || requestedDuration <= 0) throw Object.assign(new Error('视频时长必须是大于 0 秒的有效数值'), { statusCode: 400 });
  ...
  const aspectRatio = String(input.aspectRatio ?? input.aspect_ratio ?? '16:9').trim();
  const duration = requestedDuration;
  const rawQuality = input.quality !== undefined && input.quality !== null && String(input.quality).trim() !== ''
    ? input.quality
    : input.resolution;
  const quality = rawQuality === undefined || rawQuality === null || String(rawQuality).trim() === ''
