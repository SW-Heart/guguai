...
export function normalizeViralInput(input = {}, { allowDraft = false } = {}) {
  ...
  const units = (Array.isArray(input.units) ? input.units : []).map((item, index) => {
    ...
    const sourceRange = item.sourceRange && typeof item.sourceRange === 'object'
      ? { startSeconds: Math.max(0, Number(item.sourceRange.startSeconds) || 0), endSeconds: Math.max(0, Number(item.sourceRange.endSeconds) || 0) }
      : null;
    return { id: text(item.id || randomUUID(), 80, '分段 ID'), title: text(item.title || `第 ${index + 1} 段`, 80, '分段名称'), modelId, duration,
      ...
      quality: modelId === 'seedance-2.5' && item.quality === '1080p' ? '1080p' : item.quality === '480p' ? '480p' : '720p', prompt: text(item.prompt, 30000, '完整提示词'), referenceAssetIds: ids,
      ...(sourceRange && sourceRange.endSeconds > sourceRange.startSeconds ? { sourceRange } : {}),
      ...(Array.isArray(item.timelineSpanIds) ? { timelineSpanIds: item.timelineSpanIds.map(String).slice(0, 200) } : {}),
      ...(item.promptHash ? { promptHash: text(item.promptHash, 128, '提示词版本') } : {}), };
  });
...
export const planApprovalHash = (project, snapshot, options = {}) => fingerprint({ workflow: project.workflowVersion, sourceObservation: project.sourceObservation ? sourceObservationHash(project.sourceObservation) : null, input: normalizeViralInput(project, options), snapshot });

function sourceObservationHash(observation) {
...
export function assertViralReady(project, snapshot, { forGeneration = false } = {}) {
  ...
  if (project.units.some(unit => [...unit.prompt].length > 4096)) throw viralError('每段提示词最多 4096 字，请精简后确认方案');
  if (project.units.some(unit => unit.sourceRange && unit.sourceRange.endSeconds - unit.sourceRange.startSeconds > unit.duration + 0.01)) throw viralError('原片分段超过当前模型时长，请重新分析或拆分分段');
...
export const viralPlanSystem = `你是视频复刻方案整理员。你收到的是用户填写的拆解，或系统根据参考视频整理出的结构化原片观察，不是视频本身。不得声称看过原视频，不猜造镜头或台词；只使用 sourceObservation 和 sourceNotes 中已有的事实，无法确认的内容保留待补充。遵循已记录的镜头、人物关系、台词顺序和硬切，只修改有明确依据的商品事实和用户指定内容。不把身份参考图背景用于场景。素材职责与实际图片编号一一对应，引用只来自该段 referenceAssetIds。
