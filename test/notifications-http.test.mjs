...
test('announcement HTTP workflow respects admin permissions and user read state', async t => {
  ...
  const child = spawn(process.execPath, ['server.mjs'], { cwd:path.resolve(new URL('..', import.meta.url).pathname), env:{ ...process.env, NODE_ENV:'development', DATA_DIR:dataDir, PORT:String(port) }, stdio:['ignore','pipe','pipe'] });
