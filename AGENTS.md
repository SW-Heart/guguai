...
- 所有用户可见的提示、气泡、错误、空状态、表单说明、按钮和页面标签都必须使用用户语言；禁止暴露产品设计规则、内部实现、流程术语、数据结构或状态名。
- 不要向用户解释系统内部何时上传、同步、编译、编排或把内容加入哪个内部区域；确有必要时，改写成直接、清晰、可执行的操作说明和结果反馈。
- 页面设计也遵循同一原则：不展示“项目资产”“执行提示词”“实际 Prompt”“同步到云端”等内部表达，统一改成用户能直接理解的“项目素材”“画面描述”“生成内容”“上传完成”等说法。
