...
export function createDramaStudio({ api, state, esc, toast, setCreditBalance, creditText, loadTasks, loadCredits, loadFiles, uploadImage, uploadAsset, confirmDelete, taskFailure, isAssetSyncing = () => false, showAssetInFolder = null }) {
  ...
  const taskDisplayLabel = id => {
    ...
    if (taskSyncing(id)) return '视频同步中…';
    if (generation?.status === 'running') return '视频生成中…';
    if (generation?.status === 'queued') return '视频排队中…';
    if (generation?.status === 'completed' && generation.assetId) return '视频文件未找到';
    return '生成后在这里预览';
  };
  ...
  function workbenchShotPreview(shot){
    ...
    const versionStrip=versions.length?`<div class="wb-preview-versions" aria-label="视频版本">${versions.map(item=>{const ready=taskLocallyReady(item.id);const syncing=taskSyncing(item.id);const versionState=videoPreviewVersionState(item.task,{ready,syncing});const failed=versionState==='failed';const missing=versionState==='missing';const pending=versionState==='pending'||versionState==='syncing';const pendingLabel=versionState==='syncing'?'视频同步中':item.task?.status==='queued'?'视频排队中':item.task?.status==='running'?'视频生成中':'视频文件未找到';const failureLabel=failed?taskFailure(item.task)?.message||'生成失败':'';const thumb=failed?`<span class="wb-preview-thumb-failed" aria-hidden="true">${generationFailureIcon}</span><span class="sr-only">生成失败：${esc(failureLabel)}，点击查看失败原因</span>`:ready?workbenchVideoMarkup(item.file):missing?`<span class="wb-preview-thumb-missing" aria-hidden="true">${generationFailureIcon}</span><span class="sr-only">${pendingLabel}</span>`:`<i class="wb-preview-thumb-loader" aria-hidden="true"></i><span class="sr-only">${pendingLabel}</span>`;return `<button type="button" class="wb-preview-thumb ${item.id===previewTaskId?'selected':''} ${failed?'is-failed':''} ${missing?'is-missing':''} ${pending?'is-pending':''}" data-wb-preview-video="${item.id}" data-wb-preview-shot="${shot.id}" aria-label="点击查看${esc(shot.title)}视频版本${failed?'，生成失败，查看失败原因':ready?'':missing?'，视频文件未找到':`，${pendingLabel}，点击在上方查看生成进度`}" title="${failed?'查看生成失败原因':ready?'点击查看大视频':missing?'视频文件未找到':`${pendingLabel} · 点击查看生成进度`}" ${missing?'disabled':''}><span class="${failed||ready||missing?'':'wb-preview-thumb-pending'}">${thumb}</span></button>`;}).join('')}</div>`:'';
  ...
  function workbenchPreviewPanel(shot,locked,completed){
    ...
    const media=selectedFile?`<button type="button" class="wb-preview-media" data-wb-preview-file="${esc(selectedFile.id)}" aria-label="点击查看${esc(shot?.title||'分镜')}大视频">${workbenchVideoMarkup(selectedFile)}<span class="wb-preview-expand">点击查看大视频</span></button>`:selectedState==='failed'?generationFailurePreviewMarkup(selectedTask):selectedState==='pending'?workbenchVideoProgressMarkup(selectedTask)||`<div class="wb-preview-empty"><span class="wb-preview-play">${PREVIEW_PLAY_ICON}</span><b>${taskDisplayLabel(shot?.selectedVideoTaskId)}</b><small>${shot?esc(shot.title):'选择一个分镜'}</small></div>`:selectedState==='syncing'?`<div class="wb-preview-empty"><span class="wb-preview-play">${PREVIEW_PLAY_ICON}</span><b>视频同步中…</b><small>${shot?esc(shot.title):'选择一个分镜'}</small></div>`:`<div class="wb-preview-empty"><span class="wb-preview-play">${PREVIEW_PLAY_ICON}</span><b>${finalFile&&assetSyncing(finalFile)?'成片同步中…':taskDisplayLabel(shot?.selectedVideoTaskId)}</b><small>${shot?esc(shot.title):'选择一个分镜'}</small></div>`;
    return `<header class="wb-preview-head"><div><span class="wb-eyebrow">PREVIEW</span><h2>预览</h2></div><span class="wb-preview-count">${completed}/${project.shots.length} 已生成</span></header><div class="wb-preview-stage" style="--video-ratio:${fallbackRatio}">${media}</div><section class="wb-preview-strip"><header><b>视频版本</b><span>点击查看大视频</span></header><div>${versions.length?versions.map(item=>{const ready=taskLocallyReady(item.id);const syncing=taskSyncing(item.id);const versionState=videoPreviewVersionState(item.task,{ready,syncing});const failed=versionState==='failed';const missing=versionState==='missing';const pending=versionState==='pending'||versionState==='syncing';const pendingLabel=versionState==='syncing'?'视频同步中':item.task?.status==='queued'?'视频排队中':'视频生成中';const failureLabel=failed?taskFailure(item.task)?.message||'生成失败':'';const thumb=failed?`<span class="wb-preview-thumb-failed" aria-hidden="true">${generationFailureIcon}</span><span class="sr-only">生成失败：${esc(failureLabel)}，点击查看失败原因</span>`:ready?workbenchVideoMarkup(item.file):missing?`<span class="wb-preview-thumb-missing" aria-hidden="true">${generationFailureIcon}</span><span class="sr-only">任务记录不可用</span>`:`<i class="wb-preview-thumb-loader" aria-hidden="true"></i><span class="sr-only">${pendingLabel}</span>`;return `<button type="button" class="wb-preview-thumb ${item.shot.id===shot?.id&&item.id===shot?.selectedVideoTaskId?'selected':''} ${failed?'is-failed':''} ${missing?'is-missing':''} ${pending?'is-pending':''}" data-wb-preview-video="${item.id}" data-wb-preview-shot="${item.shot.id}" aria-label="点击查看${esc(item.shot.title)}大视频${failed?'，生成失败，查看失败原因':ready?'':missing?'，任务记录不可用':`，${pendingLabel}`}" title="${failed?'查看生成失败原因':ready?'点击查看大视频':missing?'任务记录不可用':pendingLabel}" ${missing?'disabled':''}><span class="${failed||ready||missing?'':'wb-preview-thumb-pending'}">${thumb}</span></button>`;}).join(''):'<p>生成视频后，缩略图会显示在这里</p>'}</div></section>${finalFile&&!assetSyncing(finalFile)?`<section class="wb-final-cut"><header><div><b>完整成片</b><span>已合成 ${project.shots.length} 个分镜</span></div><span class="wb-lock-mark">⌁ 已锁定</span></header><button type="button" class="wb-final-media" data-wb-preview-file="${esc(finalFile.id)}" aria-label="点击查看完整成片">${workbenchVideoMarkup(finalFile)}<span class="wb-preview-expand">点击查看完整成片</span></button></section>`:`<section class="wb-preview-tip"><span>⌁</span><p>${finalFile&&assetSyncing(finalFile)?'完整成片正在同步到本地，请稍后预览。':project.shots.length>2&&!locked?'所有分镜视频完成后，可点击缩略图查看大视频。':'生成多个分镜后，可点击缩略图查看大视频。'}</p></section>`}`;
  }
  ...
  function videoVersion(shot,id){const generation=task(id);const file=taskAsset(id);const selected=shot.selectedVideoTaskId===id;const syncing=taskSyncing(id);const ready=taskLocallyReady(id);const missing=generation?.status==='completed'&&Boolean(generation.assetId)&&!file;const status=generation?.status==='failed'?generationFailureMarkup(generation):syncing?'同步中…':missing?'成品文件未找到':generation?({queued:'排队中',running:'生成中'}[generation.status]||generation.status):'记录缺失';return `<button class="video-version ${selected?'selected':''}" data-select-video="${id}" data-shot-id="${shot.id}" ${ready?'':'disabled'}>${ready?workbenchVideoMarkup(file):`<span>${status}</span>`}<i>${selected&&ready?'✓ 当前版本':ready?'选择此版':syncing?'同步中…':missing?'文件未保存到本机':'等待完成'}</i></button>`;}
  ...
  function professionalVideoCostState(shot){
    ...
    if(professionalVideoUsesDynamicQuote(shot)){
      ...
      return {ready:false,credits:null,label:'…',message:'正在获取当前线路价格',priceVersion:''};
    }
