...
export function createDramaStudio({ api, state, esc, toast, setCreditBalance, creditText, loadTasks, loadCredits, loadFiles, uploadImage, uploadAsset, confirmDelete, taskFailure, isAssetSyncing = () => false }) {
  ...
  async function openProject(id) { try { const result=await api(`/api/drama/projects/${id}`); professionalPreviewTaskIds.clear(); project=normalizeProjectData(result.project); projectAssetIds=[...project.projectAssetIds]; projectAssetCategories=new Map(Object.entries(project.projectAssetCategories||{})); viewStep=project.step; scriptDraft=null; state.dramaProject=project; setStudioVisible(true); syncProjectHeader(); modelState(); render(); }catch(error){toast(error.message)} }
  async function closeProject(){ await flushSave(); resetWorkbenchVideoObserver(); resetVirtualShotWindow(); virtualShotHeights.clear(); virtualShotProjectId=''; professionalPreviewTaskIds.clear(); project=null; viewStep=null; scriptDraft=null; assetPickerShotId=''; state.dramaProject=null; syncProjectHeader(); renderProjects(); }
  ...
  function renderProfessionalShotWindow({scroll=root.querySelector('.wb-shot-scroll'), force=false}={}){
    ...
    project.shots.slice(range.start, range.end).forEach(shot => shotPreviewSignatures.set(shot.id, `${shotPreviewRenderSignatureFromMaps(shot, taskById, fileById, assetSyncing)}|preview:${professionalPreviewTaskIds.get(shot.id)||''}`));
  ...
  function renderProfessionalWorkspace({focus=true}={}){
    ...
    project.shots.forEach(shot=>shotPreviewSignatures.set(shot.id,`${shotPreviewRenderSignatureFromMaps(shot,taskById,fileById,assetSyncing)}|preview:${professionalPreviewTaskIds.get(shot.id)||''}`));
  ...
  function workbenchPreviewPanel(shot,locked,completed){
    ...
    const media=selectedFile?`<button type="button" class="wb-preview-media" data-wb-preview-file="${esc(selectedFile.id)}" aria-label="点击查看${esc(shot?.title||'分镜')}大视频">${workbenchVideoMarkup(selectedFile)}<span class="wb-preview-expand">点击查看大视频</span></button>`:workbenchVideoProgressMarkup(selectedTask)||`<div class="wb-preview-empty"><span class="wb-preview-play">${PREVIEW_PLAY_ICON}</span><b>${finalFile&&assetSyncing(finalFile)?'成片同步中…':taskDisplayLabel(shot?.selectedVideoTaskId)}</b><small>${shot?esc(shot.title):'选择一个分镜'}</small></div>`;
    return `<header class="wb-preview-head"><div><span class="wb-eyebrow">PREVIEW</span><h2>预览</h2></div><span class="wb-preview-count">${completed}/${project.shots.length} 已生成</span></header><div class="wb-preview-stage" style="--video-ratio:${fallbackRatio}">${media}</div><section class="wb-preview-strip"><header><b>视频版本</b><span>点击查看大视频</span></header><div>${versions.length?versions.map(item=>{const ready=taskLocallyReady(item.id);const syncing=taskSyncing(item.id);const pendingLabel=syncing?'视频同步中':item.task?.status==='queued'?'视频排队中':'视频生成中';return `<button type="button" class="wb-preview-thumb ${item.shot.id===shot?.id&&item.id===shot?.selectedVideoTaskId?'selected':''}" data-wb-preview-video="${item.id}" data-wb-preview-shot="${item.shot.id}" aria-label="点击查看${esc(item.shot.title)}大视频${ready?'':`，${pendingLabel}`}" ${ready?'':'disabled'}><span class="${ready?'':'wb-preview-thumb-pending'}">${ready?workbenchVideoMarkup(item.file):`<i class="wb-preview-thumb-loader" aria-hidden="true"></i><span class="sr-only">${pendingLabel}</span>`}</span></button>`;}).join(''):'<p>生成视频后，缩略图会显示在这里</p>'}</div></section>${finalFile&&!assetSyncing(finalFile)?`<section class="wb-final-cut"><header><div><b>完整成片</b><span>已合成 ${project.shots.length} 个分镜</span></div><span class="wb-lock-mark">⌁ 已锁定</span></header><button type="button" class="wb-final-media" data-wb-preview-file="${esc(finalFile.id)}" aria-label="点击查看完整成片">${workbenchVideoMarkup(finalFile)}<span class="wb-preview-expand">点击查看完整成片</span></button></section>`:`<section class="wb-preview-tip"><span>⌁</span><p>${finalFile&&assetSyncing(finalFile)?'完整成片正在同步到本地，请稍后预览。':project.shots.length>2&&!locked?'所有分镜视频完成后，可在底部合成完整成片。':'生成多个分镜后，可点击缩略图查看大视频。'}</p></section>`}`;
  }
  ...
  function patchProfessionalTaskSurfaces(){
    ...
    const selectedStatus=taskDisplayStatus(shot.selectedVideoTaskId);
      ...
      const signature=`${shotPreviewRenderSignatureFromMaps(shot,taskById,fileById,assetSyncing)}|preview:${professionalPreviewTaskIds.get(shot.id)||''}`;
