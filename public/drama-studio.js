...
export function createDramaStudio({ api, state, esc, toast, setCreditBalance, creditText, loadTasks, loadCredits, loadFiles, uploadImage, uploadAsset, confirmDelete, taskFailure, isAssetSyncing = () => false }) {
  ...
  function workbenchShotPreview(shot){
    ...
    const versions=(shot?.videoVersions||[]).map(id=>({id,file:taskAsset(id),task:task(id)})).filter(item=>item.task?.status!=='failed');
  ...
  function workbenchPreviewPanel(shot,locked,completed){
    ...
    const versions=project.shots.flatMap(item=>(item.videoVersions||[]).map(id=>({shot:item,task:task(id),file:taskAsset(id),id}))).filter(item=>(item.file||item.task)&&item.task?.status!=='failed');
