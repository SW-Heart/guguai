...
function shotPreviewRenderSignatureFromMaps(shot, taskById, fileById, isAssetSyncing = () => false) {
  ...
  return [shot?.selectedVideoTaskId || '', ...ids.map(id => {
    ...
    return `${id}:${generation?.status || ''}:${generation?.assetId || ''}:${file?.url || ''}:${file?.localStatus || ''}:${isAssetSyncing(file) ? 'syncing' : ''}`;
  })].join('|');
}
...
export function createDramaStudio({ api, state, esc, toast, setCreditBalance, creditText, loadTasks, loadCredits, loadFiles, uploadImage, uploadAsset, confirmDelete, taskFailure, isAssetSyncing = () => false }) {
  ...
  const taskLocallyReady = id => {
    ...
    return generation?.status === 'completed' && Boolean(file) && !assetSyncing(file);
  };
  const taskDisplayStatus = id => taskSyncing(id) ? 'running' : task(id)?.status;
  const taskDisplayLabel = id => taskSyncing(id) ? '视频同步中…' : task(id)?.status === 'running' ? '视频生成中…' : task(id)?.status === 'queued' ? '视频排队中…' : '生成后在这里预览';
  ...
  function renderProfessionalShotWindow({scroll=root.querySelector('.wb-shot-scroll'), force=false}={}){
    ...
    project.shots.slice(range.start, range.end).forEach(shot => shotPreviewSignatures.set(shot.id, shotPreviewRenderSignatureFromMaps(shot, taskById, fileById, assetSyncing)));
  ...
  function renderProfessionalWorkspace({focus=true}={}){
    ...
    project.shots.forEach(shot=>shotPreviewSignatures.set(shot.id,shotPreviewRenderSignatureFromMaps(shot,taskById,fileById,assetSyncing)));
  ...
  function workbenchShotPreview(shot){
    ...
    const selectedFile=taskLocallyReady(shot?.selectedVideoTaskId)?taskAsset(shot?.selectedVideoTaskId):null;
    const versions=(shot?.videoVersions||[]).map(id=>({id,file:taskAsset(id),task:task(id)}));
    const media=selectedFile?`<button type="button" class="wb-preview-media" data-wb-preview-file="${esc(selectedFile.id)}" aria-label="点击查看${esc(shot?.title||'分镜')}大视频">${workbenchVideoMarkup(selectedFile)}<span class="wb-preview-expand">点击查看大视频</span></button>`:`<div class="wb-preview-empty"><span class="wb-preview-play">${PREVIEW_PLAY_ICON}</span><b>${taskDisplayLabel(shot?.selectedVideoTaskId)}</b></div>`;
    const versionStrip=versions.length?`<div class="wb-preview-versions" aria-label="视频版本">${versions.map(item=>{const ready=taskLocallyReady(item.id);const syncing=taskSyncing(item.id);return `<button type="button" class="wb-preview-thumb ${item.id===shot.selectedVideoTaskId?'selected':''}" data-wb-preview-video="${item.id}" data-wb-preview-shot="${shot.id}" aria-label="点击查看${esc(shot.title)}第${String(versions.indexOf(item)+1).padStart(2,'0')}版大视频" ${ready?'':'disabled'}><span>${ready?workbenchVideoMarkup(item.file):`<i>${syncing?'同步中…':item.task?.status==='failed'?'生成失败':'等待生成'}</i>`}</span><b>${String(versions.indexOf(item)+1).padStart(2,'0')}</b></button>`;}).join('')}</div>`:'';
  ...
  function workbenchFinalCut(assetId){
    ...
    if(assetSyncing(file))return `<section class="wb-final-cut"><header><div><b>分镜合成</b><span>正在同步到本地</span></div><span class="wb-lock-mark">生成中</span></header><div class="wb-preview-empty"><span class="wb-preview-play">${PREVIEW_PLAY_ICON}</span><b>完整成片同步中…</b></div></section>`;
  ...
  function workbenchPreviewPanel(shot,locked,completed){
    ...
    const selectedVideoFile=taskLocallyReady(shot?.selectedVideoTaskId)?taskAsset(shot?.selectedVideoTaskId):null;
    ...
    const media=selectedFile?`<button type="button" class="wb-preview-media" data-wb-preview-file="${esc(selectedFile.id)}" aria-label="点击查看${esc(shot?.title||'分镜')}大视频">${workbenchVideoMarkup(selectedFile)}<span class="wb-preview-expand">点击查看大视频</span></button>`:`<div class="wb-preview-empty"><span class="wb-preview-play">${PREVIEW_PLAY_ICON}</span><b>${finalFile&&assetSyncing(finalFile)?'成片同步中…':taskDisplayLabel(shot?.selectedVideoTaskId)}</b><small>${shot?esc(shot.title):'选择一个分镜'}</small></div>`;
    return `<header class="wb-preview-head"><div><span class="wb-eyebrow">PREVIEW</span><h2>预览</h2></div><span class="wb-preview-count">${completed}/${project.shots.length} 已生成</span></header><div class="wb-preview-stage" style="--video-ratio:${fallbackRatio}">${media}</div><section class="wb-preview-strip"><header><b>视频版本</b><span>点击查看大视频</span></header><div>${versions.length?versions.map(item=>{const ready=taskLocallyReady(item.id);const syncing=taskSyncing(item.id);return `<button type="button" class="wb-preview-thumb ${item.shot.id===shot?.id&&item.id===shot?.selectedVideoTaskId?'selected':''}" data-wb-preview-video="${item.id}" data-wb-preview-shot="${item.shot.id}" aria-label="点击查看${esc(item.shot.title)}大视频" ${ready?'':'disabled'}><span>${ready?workbenchVideoMarkup(item.file):`<i>${syncing?'同步中…':item.task?.status==='failed'?'生成失败':'等待生成'}</i>`}</span><b>${String(item.shot.shotNumber).padStart(2,'0')}</b></button>`;}).join(''):'<p>生成视频后，缩略图会显示在这里</p>'}</div></section>${finalFile&&!assetSyncing(finalFile)?`<section class="wb-final-cut"><header><div><b>完整成片</b><span>已合成 ${project.shots.length} 个分镜</span></div><span class="wb-lock-mark">⌁ 已锁定</span></header><button type="button" class="wb-final-media" data-wb-preview-file="${esc(finalFile.id)}" aria-label="点击查看完整成片">${workbenchVideoMarkup(finalFile)}<span class="wb-preview-expand">点击查看完整成片</span></button></section>`:`<section class="wb-preview-tip"><span>⌁</span><p>${finalFile&&assetSyncing(finalFile)?'完整成片正在同步到本地，请稍后预览。':project.shots.length>2&&!locked?'所有分镜视频完成后，可在底部合成完整成片。':'生成多个分镜后，可点击缩略图查看大视频。'}</p></section>`}`;
  }
  ...
  function bindWorkbenchPreviewActions(scope=root){
    ...
    scope.querySelectorAll('[data-wb-preview-video]').forEach(button=>button.addEventListener('click',event=>{event.stopPropagation();professionalPreviewShotId=button.dataset.wbPreviewShot;professionalShotId=button.dataset.wbPreviewShot;const shot=project.shots.find(item=>item.id===professionalShotId);const selectedFile=taskAsset(button.dataset.wbPreviewVideo);const canPreview=taskLocallyReady(button.dataset.wbPreviewVideo);activateProfessionalShot(professionalPreviewShotId);if(shot&&canPreview&&selectedFile){shot.selectedVideoTaskId=button.dataset.wbPreviewVideo;queueProfessionalSave();patchProfessionalTaskSurfaces();openProfessionalMediaPreview(selectedFile.id);}}));
  }
  ...
  function patchProfessionalTaskSurfaces(){
    ...
    const selectedStatus=taskDisplayStatus(shot.selectedVideoTaskId);
      ...
      const signature=shotPreviewRenderSignatureFromMaps(shot,taskById,fileById,assetSyncing);
  ...
  function paintProjectAssetDialog(){
    const dialog=document.querySelector('#projectAssetDialog');if(!dialog)return;const query=projectAssetQuery.trim().toLocaleLowerCase('zh-CN');const files=state.files.filter(file=>['image','video','audio'].includes(file.kind)&&!assetSyncing(file)&&(!query||String(file.name).toLocaleLowerCase('zh-CN').includes(query)));const chosen=new Set(projectAssetSelection);const categoryLabel={characters:'人物',locations:'场景',props:'物品',other:'其他'}[projectAssetCategory]||'其他';
  ...
  async function selectVideo(shotId,taskId){const shot=project.shots.find(x=>x.id===shotId);if(!shot||!taskLocallyReady(taskId))return taskSyncing(taskId)?toast('视频正在同步到本地，请稍后选择'):undefined;shot.selectedVideoTaskId=taskId;await patch({shots:project.shots});}
