...
const taskErrorText = value => /\[object Object\]/.test(String(value || '')) ? '模型请求失败，供应商未返回可读的错误原因。请调整参数后重新生成。' : String(value || '');
let toastTimer;
...
function taskCard(task) {
  ...
  const stageTrack = '';
  const failureReason = task.status === 'failed' ? taskErrorText(task.error).split('。')[0] : '';
  const media = asset
    ...
    : task.status === 'failed'
      ? `<div class="card-failure"><svg viewBox="0 0 24 24"><path d="M12 8v5M12 17h.01"/><path d="M10.3 3.7 2.6 17a2 2 0 0 0 1.7 3h15.4a2 2 0 0 0 1.7-3L13.7 3.7a2 2 0 0 0-3.4 0Z"/></svg><b>生成失败</b><p>${esc(failureReason || '模型未返回可用结果')}</p></div>`
      : `<div class="card-placeholder ${task.status}" aria-hidden="true"><div class="skeleton-frame"><i></i><i></i><i></i></div>${stageTrack}</div>`;
  const spec = task.type === 'image' ? (task.size || '1:1') : `${task.aspectRatio || '16:9'} · ${task.duration || 10}s`;
  ...
  const failedAction = task.status === 'failed' ? `<button class="failure-retry task-action" type="button" data-action="continue" data-task-id="${task.id}"><svg viewBox="0 0 24 24"><path d="M20 11a8 8 0 1 0-2.3 5.7M20 4v7h-7"/></svg>调整后重试</button>` : '';
  return `<article class="task-card ${task.status}" data-record-id="${task.id}"><div class="card-visual">${media}<button class="media-open open-task" type="button" data-task-id="${task.id}" aria-label="查看${task.type === 'image' ? '商品图' : '商品视频'}详情"></button>${completedActions}${failedAction}</div><footer class="card-meta"><span class="status-pill"><i></i>${statusText(task.status)}</span><code>${esc(spec)}</code><time>${dateText(task.createdAt)}</time></footer></article>`;
}
...
function addTaskReference(task, target=task.type) { if (!task?.assetId || task.type !== 'image') return false; if (!state.refs[target].includes(task.assetId)) state.refs[target] = [task.assetId, ...state.refs[target]].slice(0, 7); renderReferences(); return true; }
...
function openGenerationDetail(id) {
  ...
  const media = asset ? (task.type === 'image' ? `<img src="${asset.url}" alt="${esc(asset.name)}">` : `<video src="${asset.url}" controls autoplay></video>`) : `<div class="detail-placeholder ${task.status}" aria-hidden="true"><div class="loader-ring"></div></div>`;
  ...
  $('#continueGeneration').disabled = !asset && task.status !== 'failed';
  $('#useGenerationReference').classList.toggle('hidden', !asset || task.type !== 'image');
  $('#deriveGeneration').classList.toggle('hidden', !asset); $('#deriveGeneration').textContent = task.type === 'image' ? '生成视频' : '生成同款';
...
$('#generationDetailDialog').addEventListener('close', () => { $('#generationDetailMedia').innerHTML = ''; state.detailTaskId = null; });
$('#continueGeneration').onclick = () => continueFromTask(state.tasks.find(item => item.id === state.detailTaskId));
$('#useGenerationReference').onclick = () => continueFromTask(state.tasks.find(item => item.id === state.detailTaskId), 'image', true);
$('#deriveGeneration').onclick = () => { const task = state.tasks.find(item => item.id === state.detailTaskId); continueFromTask(task, task?.type === 'image' ? 'video' : 'video', task?.type === 'image'); };
...
function createUploadRow(file) { const queue = $('#referenceUploads'); queue.classList.remove('hidden'); const row = document.createElement('div'); row.className = 'upload-item'; const previewUrl = URL.createObjectURL(file); row.innerHTML = `<img src="${previewUrl}" alt=""><div><b>${esc(file.name)}</b><span>准备上传</span><div class="upload-track"><i></i></div></div><strong>0%</strong>`; queue.prepend(row); return { row, previewUrl, status:row.querySelector('span'), bar:row.querySelector('.upload-track i'), percent:row.querySelector('strong') }; }
function updateUploadRow(view, percent, status, mode='') { const value = Math.max(0, Math.min(100, Math.round(percent))); view.bar.style.width = `${value}%`; view.percent.textContent = mode === 'error' ? '失败' : `${value}%`; view.status.textContent = status; view.row.classList.toggle('failed', mode === 'error'); view.row.classList.toggle('completed', mode === 'completed'); }
...
$('#imagePrompt').oninput = event => $('#imagePromptCount').textContent = `${Array.from(event.target.value).length} 字`; $('#videoPrompt').oninput = event => $('#videoPromptCount').textContent = Array.from(event.target.value).length;
async function submitGeneration(type, form, payload) { const button = form.querySelector('.generate'); const original = button.innerHTML; button.disabled = true; button.innerHTML = '<span class="button-spinner"></span><span>正在提交</span>'; try { const result = await api('/api/generations', { method:'POST', body:JSON.stringify({ type, referenceAssetIds:state.refs[type], ...payload }) }); setCreditBalance(result.balance); form.querySelector('textarea').value = ''; state.refs[type] = []; renderReferences(); toast(`任务已提交，扣除 ${result.creditCost} 积分`); await loadTasks(); } catch (error) { toast(error.message); await loadCredits(); } finally { button.disabled = false; button.innerHTML = original; } }
...
$('#videoForm').onsubmit = event => { event.preventDefault(); submitGeneration('video', event.currentTarget, { prompt:$('#videoPrompt').value, aspectRatio:$('#videoAspect').value, duration:Number($('#videoDuration').value) }); };
